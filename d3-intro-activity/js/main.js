 ...
 d3.csv('data/disasters.csv') //ASYNCHRONOUS LOADING
  .then(data => {
	...
	//		fill:  
		...
		.append("circle")
			.attr("opacity", 0.8)
			.attr("r", (d.cost/165 * (100) + 5))  //mapping from domain (cost) to range (pixels))
			.attr("cx", d.daysFromYrStart/365 * (timelineX2-timelineX1) + timelineX1)  //mapping from domain (days) to range (pixels 'x')
			.attr("cy", (2017 - d) * gapBetweenTimelines + startOfTimelinesY ) //mapping from domain (year) to range (pixels 'y')
			.attr("fill", d => {
				if (d.category == "tropical-cyclone") {
					return "#081d58";
				} else if (d.category == "drought-wildfire") {
